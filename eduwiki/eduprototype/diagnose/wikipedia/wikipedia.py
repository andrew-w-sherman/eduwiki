...
from .exceptions import PageError, DisambiguationError, RedirectError
...
Change the language of the API being requested.
    ...
    After setting the language, the cache for ``search``, ``suggest``,
    and ``summary`` will be cleared.

    .. note:: Make sure you search for page titles in the
    language that you have set.
    '''
    global API_URL
...
def set_rate_limiting(rate_limit, min_wait=timedelta(milliseconds=50)):
    ...
    * min_wait - if rate limiting is enabled, `min_wait` is a timedelta
    ...
    if not rate_limit:
        ...
        RATE_LIMIT_LIMIT_MIN_WAIT = min_wait
...
def page(title=None, pageid=None, auto_suggest=True, redirect=True,
    ...
    * auto_suggest - let Wikipedia find a valid page title for the query
    * redirect - allow redirection without raising RedirectError
    * preload - load content, summary, images, references,
    and links during initialization
    '''
...
class WikipediaPage(object):
    ...
    def load(self, redirect=True, preload=False):
        ...
        # then the page must be a disambiguation page
            ...
            links = [li.a[u'href'].split('/')[-1].replace("_", "+")
                     for li in filtered_lis if li.a]

            raise DisambiguationError(getattr(self, 'title', data['title']),
                                      may_refer_to, descriptions, links)
    ...
    def html(self):
        ...
        if not getattr(self, '_html', False):
            ...
            request = _wiki_request(**query_params)
            self._html = \
                request['query']['pages'][self.pageid]['revisions'][0]['*']
    ...
    def wikitext(self):
        ...
        if not getattr(self, '_wikitext', False):
            query_params = {
                'prop': 'revisions',
                'rvprop': 'content',
                'rvlimit': 1,
    ...
    def content(self):
        '''
        Plain text content of the page, excluding images,
        ...
        if not getattr(self, '_content', False):
            ...
            self._revision_id = \
                request['query']['pages'][self.pageid]['revisions'][0]['revid']
            self._parent_id = \
                request['query']['pages'][self.pageid]['revisions'][0]['parentid']
    ...
    def images(self):
        ...
        if not getattr(self, '_images', False):
            ...
            self._images = [image['imageinfo'][0]['url']
                            for image in images if image.get('imageinfo')]
    ...
    def references(self):
        ...
        May include external links within page that aren't
    ...
    def links(self):
        ...
        if not getattr(self, '_links', False):
            ...
            while True:
                ...
                self._links.extend([link['title'] for link in
                                    request['query']['pages']
                                    [self.pageid]['links']])
    ...
    def backlinks(self):
        '''
        List of titles of Wikipedia page backlinks on a page.
        ...
        if not getattr(self, '_backlinks', False):
            ...
            if not getattr(self, 'title', None) is None:
                request['bltitle'] = self.title
            else:
                request['blpageid'] = self.pageid
            lastContinue = {}
            ...
            while True:
                ...
                request = _wiki_request(**params)
                self._backlinks.extend([bl['title'] for bl in request['query']['backlinks']])

                if 'continue' not in request:
                    break

                lastContinue = request['blcontinue']
        ...
        if not getattr(self, '_categories', False):
            ...
            while True:
                ...
                self._categories.extend(
                    [link['title'].replace("Category:", "")
                     for link in
                     request['query']['pages'][self.pageid]['links']])
    ...
    def section(self, section_title):
        ...
        .. warning:: Calling `section` on a section that has subheadings will
            NOT return the full text of all of the subsections. It only gets
            the text between `section_title` and the next subheading,
